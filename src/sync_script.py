...
DEFAULT_MAPPING_JSON = r'''
{
  "_meta": {
    ...
    "model_to_manufacturer_note": "model_to_manufacturer：按车型名精确匹配，强制覆盖数据源给的厂商(manufacturer)字段，在解析品牌之前应用。用于修正数据源把某款车型的厂商字段改来改去导致的『一车型对应多厂商』问题，也可用于把存量行迁移到新的厂商归属（存量规范化那一趟会把不一致的行改写过来，不需要做一次几十分钟的全量重抓）。历史：2026-08 数据源把『欧拉好猫』从『长城新能源』改挂到『长城汽车』，当时以为只是这一款车挑窝，曾按『长城新能源』的历史一致性把它钉回去；2026-09 全量重抓后发现数据源已经整体废弃了『长城新能源』这个厂商名（欧拉芭蕾猫、欧拉闪电猫也一并挪到了『长城汽车』），原判断的前提不成立，故改为跟随数据源、统一归到『长城汽车』。这一条在存量行迁移完成后就是与数据源一致的空操作，之后可以安全删除。"
  },
