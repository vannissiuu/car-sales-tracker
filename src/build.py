...
.switch.disabled{cursor:not-allowed;opacity:.45;pointer-events:none;}
...
.year-legend{display:none;align-items:center;gap:14px;padding:0 4px 10px;font-size:11.5px;color:var(--text-secondary);flex-wrap:wrap;}
.year-legend-item{display:flex;align-items:center;gap:5px;}
...
<div class="wrap">
  ...
  <div class="card controls">
    <div class="ctrl-group">
      ...
      <div class="chip-disabled-hint" id="yearDisabledHint" style="display:none;">年度视图已含全部年份</div>
    </div>
    <div class="ctrl-group">
    ...
    <div class="ctrl-group">
      ...
      <div class="chip-disabled-hint" id="stackedDisabledHint" style="display:none;">年视图不支持堆积</div>
    </div>
    <div class="ctrl-group">
  ...
  <div class="footnote" id="footnoteMonth">
    折线图：默认展示当年 Top 20（按年初至今累计销量排名），前 8 名使用可辨识色，其余以及「其他」聚合线为淡灰色，
    悬停/点击线条或图例可高亮；勾选右侧列表可手动增补关注的对象。当年数据只到最新已发布月份，后续月份不补零，折线在此处断开。
    点击任意折线或图例条目可在右侧查看该对象的月度明细、同比、统计范围与相关动态。
    悬停「厂商」「品牌」粒度的折线或图例，会额外提示该对象当前统计范围包含哪些车型；「车型」是本工具的最小统计单位，不再细分。
  </div>
  <div class="footnote" id="footnoteYear" style="display:none;">
    年度对比图：横轴为对象（按最新年份同期值降序），每个对象下三根柱依次是各年份，颜色由淡到浓表示年份由早到新。
    <b>完整年份的柱高是全年合计，最新的不完整年份柱高是同期合计（口径写在横轴标签上）</b>；
    完整年份柱内的横线是该年<b>同期值</b>，用来跟最新年份的柱顶直接比高矮——这是判断"今年能不能追平"的依据。
...
// 年视图下，图例数值/抽屉/悬浮构成提示等"当前参照年份"一律用最新年份，绝不修改 state.year
// （切回按月视图时 state.year 要原样保留用户之前选的年份）。按月视图下这个函数就是 state.year 本身。
function refYear(){
  return state.viewMode==='year' ? YEARS[YEARS.length-1] : state.year;
}
...
/* ---------------- 按年对比（年视图）专用数据层 ----------------
   capMonth 完全由数据推导，不写死任何月份数字：capMonth = 最新年份(YEARS 最大值)
   实际有数据的最后一个月，用既有的 lastMonthOfYear() 取。
   完整年份（不是最新年份，或最新年份但 capMonth===12）用全年合计；
   最新且不完整的年份（capMonth<12）用 1..capMonth 同期合计。
   "同期值"（1..capMonth 累计）对每个年份都算，供参考线 / 同比使用；
   capMonth===12 时同期==全年，参考线/同期说明按规则整体隐藏。 */
function yearRangeSum(fuelArr, evArr, year, m1, m2){
  var s = 0;
  for(var m=m1;m<=m2;m++){
    var v = monthlyValueAt(fuelArr, evArr, year, m);
    if(v!=null) s += v;
  }
  return s;
}
function isYearComplete(year){ return lastMonthOfYear(year) === 12; }
function computeYearUniverse(){
  var dim = currentDim();
  var latestYear = YEARS[YEARS.length-1];
  var capMonth = lastMonthOfYear(latestYear);
  var entities = [];
  for(var i=0;i<dim.n.length;i++){
    var perYear = {};
    var anySales = false;
    YEARS.forEach(function(y){
      var complete = isYearComplete(y);
      var full = complete ? yearRangeSum(dim.f[i], dim.e[i], y, 1, 12) : null;
      var samePeriod = capMonth>0 ? yearRangeSum(dim.f[i], dim.e[i], y, 1, capMonth) : 0;
      var barVal = complete ? full : samePeriod;
      if(barVal>0) anySales = true;
      perYear[y] = {complete:complete, full:full, samePeriod:samePeriod, barVal:barVal};
    });
    ...
    entities.push({
      ...
      key: entityKey(state.gran, dim.n[i]),
      perYear: perYear,
      // 排序 / 图例数值口径：最新年份的同期值（capMonth===12 时同期==全年，两者相等）
      ytd: perYear[latestYear].samePeriod
    });
  }
  entities.sort(function(a,b){ return b.ytd - a.ytd; });
  entities.forEach(function(e,idx){ e.rank = idx+1; });
  return {entities:entities, years:YEARS, latestYear:latestYear, capMonth:capMonth};
}
// 「其他」= 年度对象池里未展示对象的合计，按年分别求和：完整年份用全年值，不完整年份用同期值
// （即每年分别加总各未展示实体的 perYear[y].barVal，口径与柱高完全一致）。
function computeOtherYear(yearUniverse, shownList){
  var shownKeys = new Set(shownList.map(function(e){return e.key;}));
  var perYear = {};
  var hasOther = false;
  yearUniverse.years.forEach(function(y){ perYear[y] = {complete:isYearComplete(y), full:0, samePeriod:0, barVal:0}; });
  yearUniverse.entities.forEach(function(e){
    if(shownKeys.has(e.key)) return;
    yearUniverse.years.forEach(function(y){
      var v = e.perYear[y];
      perYear[y].barVal += v.barVal;
      perYear[y].samePeriod += v.samePeriod;
      if(v.full!=null) perYear[y].full = (perYear[y].full||0) + v.full;
      if(v.barVal>0.4) hasOther = true;
    });
  });
  return {hasOther:hasOther, perYear:perYear};
}
...
function computeShownKeys(universe, topN){
  topN = topN || 20; // 按月视图 Top20；按年视图调用方传 10（展示上限 10 个对象，见年视图对象池规则）
...
function resetToTop20(){
  // 年视图下"重置"对应的是年视图自己的展示上限（10 个对象），不是按月视图的 Top20——
  // 两套池子/排序口径完全不同，必须各用各的 computeXxxUniverse()，不能混用。
  if(state.viewMode==='year'){
    var yu = computeYearUniverse();
    state.shown = new Set(yu.entities.slice(0,10).map(function(e){return e.key;}));
  } else {
...
function renderYearChips(){
  ...
  YEARS.forEach(function(y){
    ...
    c.addEventListener('click', function(){
      // 年视图下年份 chip 被禁用（见 syncControlStates()），CSS 上已经 pointer-events:none
      // 挡掉了点击，这里再加一道防线，绝不修改 state.year——年度视图本就包含全部年份，
      // 且切回按月视图时用户原来选的年份要原样恢复。
...
// 图表区上方的年份小图例：只在年视图下显示，2024/2025/2026 三格配对应浓淡的小色块
// （同一色相 + yearOpacityAt 的不透明度，跟柱子颜色来源完全一致，不另起一套颜色）。
function renderYearLegend(){
  var el = document.getElementById('yearLegend');
  if(!el) return;
  if(state.viewMode!=='year'){ el.style.display='none'; return; }
  el.style.display='flex';
  var color = cssVar('--series-1');
  el.innerHTML = YEARS.map(function(y, idx){
    var op = yearOpacityAt(idx, YEARS.length);
    // 修正9：横轴翻转成"对象"之后，原先挂在横轴标签上的口径（"2024 全年"/"2026 1–7月"）
    // 就没地方放了。口径是这张图赖以成立的前提，绝不能丢——移到年份图例上。
    // capMonth===12 时同期==全年，统一写"全年"。
    var cap = lastMonthOfYear(YEARS[YEARS.length-1]);
    var cal = (cap===12 || lastMonthOfYear(y)===12) ? ' 全年' : (' 1–'+cap+'月');
    return '<span class="year-legend-item"><span class="year-legend-swatch" style="background:'+color+';opacity:'+op+';"></span>'+escapeHtml(String(y)+cal)+'</span>';
  }).join('');
}
function syncControlStates(){
  ...
  document.querySelectorAll('#yearChips .chip').forEach(function(c){
    ...
    c.classList.toggle('disabled', yearDisabled);
  });
  var yearHint = document.getElementById('yearDisabledHint');
  if(yearHint) yearHint.style.display = yearDisabled ? '' : 'none';
  var stackedHint = document.getElementById('stackedDisabledHint');
  ...
  if(rsBtn) rsBtn.textContent = yearDisabled ? '重置为 Top 10' : '重置为 Top 20';
  ...
  document.getElementById('modeSwitch').classList.toggle('disabled', yearDisabled);
...
// 年视图专用：本次渲染的分组柱行数据（一个 custom series 里所有对象x年份的格子），
// 供图表 click 事件反查"点了哪个对象"。
var yearChartRows = null;
// 年视图配色：年份是有序维度，统一用同一色相（CSS 变量 --series-1），靠不透明度区分深浅，
// 不生成任何新的颜色字符串。数组长度与建议值对齐（3 个年份）；年份数不是 3 时按首尾值线性插值，
// 保持"越新越浓、单调递增"这条硬性约束。
var YEAR_OPACITIES = [0.42, 0.68, 1.0];
function yearOpacityAt(idx, total){
  if(total === YEAR_OPACITIES.length) return YEAR_OPACITIES[idx];
  if(total<=1) return 1;
  var lo = YEAR_OPACITIES[0], hi = YEAR_OPACITIES[YEAR_OPACITIES.length-1];
  return lo + (hi-lo) * idx / (total-1);
}
...
function buildSeriesMonth(universe){
...
/* ---------------- 按年对比（年视图）图表：单个 custom series + renderItem 自绘分组柱 ----------------
   ECharts 的 markLine 定位不到分组柱状图里某一根具体的柱子（只能落在类目刻度中心，拿不到组内
   偏移），所以参考线没法用 markLine 做。这里照搬 /tmp/yearview-mock/index.html 已验证过的形态：
   renderItem 里用 api.size([1,0]) 拿类目宽度，按对象数手动切分每根柱的左边界/宽度，柱子矩形、
   参考线、虚线开口顶边都用同一套坐标画，参考线因此是像素级对齐的——不是重新设计，是原样搬运。 */
function yearTooltipLines(d){
  if(d.skip){
    return [d.name+' · '+d.year+'年', '该年无销量'].join('<br/>');
  }
  var caliber = d.complete ? '全年' : ('1–'+d.capMonth+'月');
  var lines = [d.name+' · '+d.year+'年 · '+caliber];
  lines.push(caliber+'值：'+formatNum(d.value));
  if(d.complete && d.capMonth<12){
    lines.push('1–'+d.capMonth+'月同期值：'+formatNum(d.samePeriod));
  }
  if(!d.hasPrevYear){
    lines.push('同期同比：无上年同期数据');
  } else if(d.prevSamePeriod>0){
    var pct = (d.samePeriod - d.prevSamePeriod) / d.prevSamePeriod * 100;
    lines.push('同期同比：' + (pct>=0?'+':'') + pct.toFixed(1) + '%');
  } else if(d.samePeriod>0){
    lines.push('同期同比：新增');
  } else {
    lines.push('同期同比：—');
  }
...
function makeYearRenderItem(rows, yMax, textColor, refColor){
  return function(params, api){
    var d = rows[params.dataIndex];
    if(!d) return {type:'group', children:[]};
    var n = d.seriesCount, si = d.seriesIndex, catIndex = d.catIndex;

    // 分组柱布局：category band 宽度 -> 减去组间间隙 -> 按对象数均分 -> 每份再减去组内间隙
    var bandWidth = api.size([1,0])[0];
    var categoryGap = 0.28;
    var usableWidth = bandWidth * (1 - categoryGap);
    var step = usableWidth / n;
    var barGapRatio = 0.18;
    var barWidth = step * (1 - barGapRatio);
    var basePt = api.coord([catIndex, 0]);
    var groupLeft = basePt[0] - usableWidth / 2;
    var barLeft = groupLeft + si * step + (step - barWidth) / 2;
    var yBase = basePt[1];

    if(d.skip){
      // 零值柱：不画柱/参考线/端帽，只留一个透明命中区（覆盖整个柱高范围），让悬浮能弹出
      // "该年无销量" 提示；柱子位置本身自然留空，不需要额外处理。
      var topPt = api.coord([catIndex, yMax]);
      return {type:'group', children:[{
        type:'rect',
        shape:{x:barLeft, y:topPt[1], width:barWidth, height: Math.max(0, yBase-topPt[1])},
        style:{fill:'rgba(0,0,0,0)'}
      }]};
    }

    var centerPt = api.coord([catIndex, d.value]);
    var yTop = centerPt[1];
    var barHeight = Math.max(0, yBase - yTop);
    var children = [{
      type:'rect',
      shape:{x:barLeft, y:yTop, width:barWidth, height:barHeight},
      style:{fill:d.color, opacity:d.opacity}
    }];

    if(d.open){
      // 不完整年份：柱顶不封口，画一条正好落在真实柱顶上的虚线——没有任何外推，只是几何上不封口
      children.push({
        type:'line',
        shape:{x1:barLeft, y1:yTop, x2:barLeft+barWidth, y2:yTop},
        style:{stroke:d.color, lineWidth:2, lineDash:[4,3], opacity:d.opacity}
      });
    }

    // 柱顶数值标签：只在展示对象数（横轴类目数，d.catCount）<=3 时常驻，超过就只留给 tooltip，
    // 避免密集时糊成一片——注意这里不能用 n（=seriesCount=每组柱子数=年份数，固定3，跟"展示对象数"
    // 是两个概念），否则这条规则会永远生效。
    if(d.catCount<=3){
      children.push({
        type:'text',
        style:{
          text: formatNum(d.value), x: barLeft+barWidth/2, y: yTop-6,
          textAlign:'center', textVerticalAlign:'bottom', fontSize:10, fill:textColor
        }
      });
    }

    if(d.showRef){
      // 参考线：该年 1..capMonth 同期值，数值标签任何情况都不常驻（移到 tooltip 里）
      var refPt = api.coord([catIndex, d.samePeriod]);
      var ry = refPt[1];
      children.push({type:'line', shape:{x1:barLeft, y1:ry, x2:barLeft+barWidth, y2:ry},
        style:{stroke:refColor, lineWidth:2.5}});
      children.push({type:'line', shape:{x1:barLeft, y1:ry-3, x2:barLeft, y2:ry+3},
        style:{stroke:refColor, lineWidth:2}});
      children.push({type:'line', shape:{x1:barLeft+barWidth, y1:ry-3, x2:barLeft+barWidth, y2:ry+3},
        style:{stroke:refColor, lineWidth:2}});
    }
...
function renderChartYear(universe){
  ...
  var gridColor = cssVar('--grid');
  var surface = cssVar('--surface-1');
  var refColor = cssVar('--text-primary'); // 参考线：高对比中性色（浅色模式深、深色模式浅），非新写死颜色
  var barColor = cssVar('--series-1'); // 年份是有序维度：只用这一个色相，深浅靠 opacity 区分，见 yearOpacityAt

  var shownList = universe.entities.filter(function(e){ return lastShownKeys.has(e.key); });
  shownList.sort(function(a,b){ return a.rank-b.rank; });

  var otherInfo = computeOtherYear(universe, shownList);
  var showOther = otherInfo.hasOther && state.otherVisible;

  // 横轴类目 = 展示的对象，已经是 computeYearUniverse() 算好的"最新年份同期值降序"；
  // 「其他」聚合（如果开启）追加在最后，排在最右。展示上限 10 个对象（用户可手动增补更多）。
  var objects = shownList.map(function(e){
    return {key:e.key, name:e.name, perYear:e.perYear, isOther:false};
  });
  if(showOther){
    objects.push({key:'__other__', name: otherLineName().series, perYear: otherInfo.perYear, isOther:true});
  }

  var years = universe.years; // YEARS 升序：每个对象下柱子固定顺序 2024→2025→2026
  var capMonth = universe.capMonth;
  var seriesCount = years.length; // 每个对象下并排的柱子数（=年份数），用于 renderItem 里的分组布局
  var n = objects.length; // 横轴类目数（展示对象数），标注策略里的"展示对象数"看这个，不是 seriesCount
  var cats = objects.map(function(o){ return o.name; });

  var rows = [];
  var maxVal = 0;
  objects.forEach(function(obj, ci){
    years.forEach(function(y, si){
      var py = obj.perYear[y];
      var prevY = si>0 ? years[si-1] : null;
      var prevPy = prevY!=null ? obj.perYear[prevY] : null;
      var row = {
        key:obj.key, name:obj.name, isOther:obj.isOther,
        catIndex:ci, seriesIndex:si, seriesCount:seriesCount, catCount:n,
        year:y, complete:py.complete, capMonth:capMonth,
        value:py.barVal, samePeriod:py.samePeriod,
        skip: !(py.barVal>0),
        showRef: (py.complete && capMonth<12 && py.barVal>0),
        open: !py.complete,
        color: barColor,
        opacity: yearOpacityAt(si, seriesCount),
        hasPrevYear: prevY!=null,
        prevSamePeriod: prevPy ? prevPy.samePeriod : null
      };
      rows.push(row);
      if(py.barVal>maxVal) maxVal = py.barVal;
    });
  });
  yearChartRows = rows;

  var yMax = Math.ceil(maxVal*1.2/10000)*10000;
  if(yMax<=0) yMax = 10;

  // 对象数 >6 时横轴标签旋转 30°，避免中文对象名互相重叠；同时给底部多留一点空间。
  var rotateLabels = n>6;

  var option = {
    backgroundColor:'transparent',
    animationDuration:280,
    textStyle:{color:textColor, fontFamily:'inherit'},
    grid:{left:56,right:20,top:28,bottom: rotateLabels?58:36,containLabel:true},
    xAxis:{
      type:'category', data:cats,
      axisLine:{lineStyle:{color: cssVar('--baseline')}},
      axisTick:{show:false},
      axisLabel:{color:textColor, fontSize:12.5, fontWeight:600, interval:0, rotate: rotateLabels?30:0}
    },
    yAxis:{
      // custom series 不会给 y 轴提供任何数据范围，min 必须显式写 0，否则轴会塌掉、
      // 所有柱子高度看起来一样（这里踩过的坑，见需求文档）。
    ...
    series:[{
      id:'__yearSeries__',
      type:'custom',
      renderItem: makeYearRenderItem(rows, yMax, textColor, refColor),
...
chart.on('click', function(params){
  if(state.viewMode==='year'){
    ...
    if(!yearChartRows) return;
    var row = yearChartRows[params.dataIndex];
...
function buildCompNode(gran, name){
  ...
  // 年视图下这里的"当前筛选年份"必须用最新年份（refYear()），不是 state.year——
  // 年份 chip 在年视图下被禁用/不生效，state.year 只是切回按月视图时要恢复的旧选择。
  var models = entityModels(gran, name, refYear());
  ...
  if(models.length===0){
    ...
    subE.textContent = '当前筛选（'+refYear()+'年 · '+energyLabel()+'）下暂无销量数据';
...
function renderLegend(universe){
  ...
  filtered.forEach(function(e){
    ...
    dot.className='dot';
    // 年视图下对象身份不再靠颜色（颜色改为按年份编码，见 renderChartYear），图例圆点
    // 整体停用——只隐藏圆点本身，勾选框/名字/数值照常显示。年份小图例见 #yearLegend。
    if(state.viewMode==='year'){
      dot.hidden = true;
    } else {
      var dotColor = null;
      if(lastShownKeys.has(e.key)){
        var top8idx = top8Keys.indexOf(e.key); dotColor = top8idx>=0 ? palette[top8idx] : mutedColor;
      }
      dot.style.background = dotColor || 'transparent';
      dot.style.border = dotColor ? 'none' : '1px solid ' + mutedColor;
    }
    row.appendChild(dot);
  ...
  // 年视图下池子口径不一样："YEARS 里任意一年有销量"，而不是"state.year 这一年有销量"——
  // 措辞必须跟着换，否则用户会以为池子大小是按 state.year（这个年份 chip 在年视图下已经
  // 被禁用，跟当前展示无关）筛出来的。
  var poolYearDesc = state.viewMode==='year'
    ? (YEARS[0] + '–' + YEARS[YEARS.length-1] + '年 任一年有销量')
    : (state.year + '年有销量');
  document.getElementById('legendCount').textContent = state.searchTerm
    ? ('匹配 ' + filtered.length + ' 个 / 共 ' + universe.entities.length + ' 个对象（' + poolYearDesc + '）')
    : (filtered.length + ' 个对象（' + poolYearDesc + '）');
...
/* ---------------- 表格视图 ---------------- */
// 表格视图与 CSV 导出共用同一份行数据，保证两者口径一致（按月/按年各一套，视图分派）
function buildTableRowsMonth(universe){
  var shownList = universe.entities.filter(function(e){return lastShownKeys.has(e.key);});
  shownList.sort(function(a,b){return a.rank-b.rank;});
...
function renderTableMonth(universe){
  ...
  var rows = buildTableRowsMonth(universe);
...
// 年视图表格/CSV 列结构：对象名 | Y1全年 | Y1同期 | Y2全年 | Y2同期 | ... | 同期同比(最新年 vs 上一年)
// 完全由 YEARS + capMonth 动态生成，不写死年份/月份：完整年份才有"全年"列，capMonth<12 时
// 每年才有"同期"列（capMonth===12 时同期==全年，同期列整体省略，只留每年的"全年"列）。
function buildYearTableColumns(universe){
  var years = universe.years, capMonth = universe.capMonth;
  var cols = [];
  years.forEach(function(y){
    if(isYearComplete(y)){ cols.push({type:'full', year:y, header: y+'全年'}); }
    if(capMonth<12){ cols.push({type:'same', year:y, header: y+'同期（1–'+capMonth+'月）'}); }
  });
  var latestYear = years[years.length-1];
  var prevYear = years.length>=2 ? years[years.length-2] : null;
  var yoyCol = prevYear!=null
    ? {header: (capMonth<12?'同期同比':'同比') + '（'+prevYear+'→'+latestYear+'）'}
    : null;
  return {cols:cols, yoyCol:yoyCol, latestYear:latestYear, prevYear:prevYear, capMonth:capMonth};
}
function yoyCellText(curVal, prevVal){
  if(prevVal>0){
    var pct = (curVal-prevVal)/prevVal*100;
    return (pct>=0?'+':'')+pct.toFixed(1)+'%';
  }
  if(curVal>0) return '新增';
  return '—';
}
function buildTableRowsYear(universe){
  var shownList = universe.entities.filter(function(e){return lastShownKeys.has(e.key);});
  shownList.sort(function(a,b){return a.rank-b.rank;});
  var otherInfo = computeOtherYear(universe, shownList);
  var meta = buildYearTableColumns(universe);
  function rowFor(name, rank, perYear){
    var cells = meta.cols.map(function(c){
      var py = perYear[c.year];
      return c.type==='full' ? py.full : py.samePeriod;
    });
    var yoyText = null;
    if(meta.yoyCol){
      yoyText = yoyCellText(perYear[meta.latestYear].samePeriod, perYear[meta.prevYear].samePeriod);
    }
    return {rank:rank, name:name, cells:cells, yoyText:yoyText};
  }
  var rows = shownList.map(function(e){ return rowFor(e.name, '#'+e.rank, e.perYear); });
  if(otherInfo.hasOther && state.otherVisible){
    rows.push(rowFor(otherLineName().table, '', otherInfo.perYear));
  }
  return {meta:meta, rows:rows};
}
function renderTableYear(universe){
  var el = document.getElementById('tableview');
  var built = buildTableRowsYear(universe);
  var headHtml = '<th>排名</th><th>名称</th>' +
    built.meta.cols.map(function(c){return '<th>'+escapeHtml(c.header)+'</th>';}).join('') +
    (built.meta.yoyCol ? ('<th>'+escapeHtml(built.meta.yoyCol.header)+'</th>') : '');
  var html = '<table class="datatable"><thead><tr>'+headHtml+'</tr></thead><tbody>';
  built.rows.forEach(function(r){
    html += '<tr><td>'+escapeHtml(r.rank)+'</td><td>'+escapeHtml(r.name)+'</td>';
    r.cells.forEach(function(v){ html += '<td>'+(v==null?'—':formatNum(v))+'</td>'; });
    if(built.meta.yoyCol) html += '<td>'+escapeHtml(r.yoyText)+'</td>';
    html += '</tr>';
  });
  html += '</tbody></table>';
  el.innerHTML = html;
...
function csvFileNameScopeParts(){
...
function triggerCsvDownload(fname, lines){
...
function downloadCsvMonth(universe){
  if(!universe) return;
  var rows = buildTableRowsMonth(universe);
  var header = ['排名','名称'];
  ...
  var fname = '汽车销量_' + state.year + '_' + csvFileNameScopeParts().join('_') + '_' + energyLabel() + '.csv';
  triggerCsvDownload(fname, lines);
}
function downloadCsvYear(universe){
  if(!universe) return;
  var built = buildTableRowsYear(universe);
  var header = ['排名','名称'].concat(built.meta.cols.map(function(c){return c.header;}));
  if(built.meta.yoyCol) header.push(built.meta.yoyCol.header);
  var lines = [header.map(csvEscape).join(',')];
  built.rows.forEach(function(r){
    var line = [r.rank, r.name].concat(r.cells.map(function(v){ return v==null ? '' : Math.round(v); }));
    if(built.meta.yoyCol) line.push(r.yoyText);
    lines.push(line.map(csvEscape).join(','));
  });
...
function openDrawer(key, name){
  ...
  // 年视图下抽屉一律参照最新年份（refYear()），不是 state.year——年份 chip 在年视图下
  // 被禁用/不生效，state.year 只是切回按月视图时要原样恢复的旧选择，绝不能被这里读出来当真。
  var dy = refYear();
  var yearViewNote = state.viewMode==='year' ? '（年视图参照最新年份）' : '';
  ...
  var drawerSubTail = state.gran==='energy'
    ? (energyScopeLabel() + ' · ' + dy + '年' + yearViewNote)
    : (ownerLabel() + ' · ' + dy + '年' + yearViewNote + ' · ' + energyLabel());
  document.getElementById('drawerSub').textContent = granLabel() + drawerSubTail;

  var cur = findEntity(key, dy);
  var prevYear = dy - 1;
  // prevFull：上年的完整 12 个月原始数据，供逐月柱状图/表格做"同月对同月"展示（本身没有跨期问题）。
  var prevFull = YEARS.indexOf(prevYear)>=0 ? findEntity(key, prevYear) : null;
  // N = dy 年实际有数据的最后一个月（不再依赖 lastUniverse.lastMonth——年视图下 lastUniverse
  // 是年度池子，没有这个字段；lastMonthOfYear() 是纯函数，直接算，按月/按年视图都适用）。
  var lastM = lastMonthOfYear(dy);
  ...
  drawerChart.setOption({
    ...
    series:[
      {name:dy+'年', type:'bar', data:curMonthly, itemStyle:{color:palette[0], borderRadius:[3,3,0,0]}, barMaxWidth:18},
  ...
  var html = '<thead><tr><th>月份</th><th>'+dy+'年销量</th>' + (prevFull?('<th>'+prevYear+'年同月</th><th>同比</th>'):'') + '</tr></thead><tbody>';
  for(var i=0;i<12;i++){
...
function renderScope(key, name){
  ...
  el.innerHTML='';
  // 年视图下参照最新年份（refYear()），与 openDrawer 里其余年份口径保持一致，不用 state.year。
  var filterLabel = refYear()+'年 · '+energyLabel();
  ...
  if(parsed.gran==='energy'){
    ...
    var eModels = energyModels(energyType, refYear());
  ...
  var models = entityModels(parsed.gran, name, refYear());
...
function renderAll(){
  syncControlStates();
  // 按月视图：computeUniverse() 依赖 state.year；按年视图：computeYearUniverse() 自己算
  // capMonth，池子/排序都跟 state.year 无关（对象池是"YEARS 里任意一年有销量"）。
  // 展示上限也不同：按月 Top20，按年 Top10（见需求"展示上限 10 个对象"）。
  var u = state.viewMode==='year' ? computeYearUniverse() : computeUniverse();
  lastUniverse = u;
  // 修正2：交给 computeShownKeys 统一算"当前展示集合"（交集为空且未主动清空时才补 TopN，
  // 并把补的 TopN 持久化进 state.shown），不再用 state.shown.size===0 直接整体替换。
  lastShownKeys = computeShownKeys(u, state.viewMode==='year' ? 10 : 20);
  ...
  if(state.viewMode==='year'){
    ...
    document.getElementById('chartTitle').textContent =
      granLabel() + titleTail + ' · ' + YEARS[0] + '–' + u.latestYear + '年 按年对比' + periodNote;
  } else {
    document.getElementById('chartTitle').textContent =
